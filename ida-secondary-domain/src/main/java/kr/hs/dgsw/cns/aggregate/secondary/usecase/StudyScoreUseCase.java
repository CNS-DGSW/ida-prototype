...
public class StudyScoreUseCase {
    private final ScoreService scoreService;

    public void uploadStudyScore(FileRequest request) {
        scoreService.uploadScore(request, ScoreType.STUDY);
    }

    public void getStudyScore(OutputStream outputStream) {
        ExcelRequestDto requestDto = ExcelRequestDto.builder()
                .fileName("심층면접1_서식")
                .title("2023학년도 신입생 2차 전형 심층면접1 점수 일람표")
                .body("2교시   초검                    (인)   재검                    (인)   삼검                    (인)")
                .build();

        scoreService.getScore(outputStream, requestDto, ScoreType.STUDY);
    }
}
